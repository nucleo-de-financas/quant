...
from coletar_dados.b3.yahoo.seletores import *  # * importa tudo
from analise_tecnica.calculadoras import *

# Coletando Dados # ----------------------------------------------------------------------------------------------------

Ticker_trabalhado = YahooTickers.PETR4
data = YahooAPI(intervalo=YahooIntervalo.DIA_1, periodo=YahooPeriodo.ANO_1, ticker=Ticker_trabalhado).historico()
print(data)
...
Media_curto = CalculadoraMediaMovel(ativo=data, media_movel=SeletorMediaMovel.EXPONENCIAL,
                                    janela=Periodo_curto).calcular()
Media_longo = CalculadoraMediaMovel(ativo=data, media_movel=SeletorMediaMovel.EXPONENCIAL,
                                    janela=Periodo_Longo).calcular()

# Criando listas para plotar no gráfico
Lista_EMA_curto = list(Media_curto)
Lista_EMA_longo = list(Media_longo)
Lista_PRECO_fechamento = data.fechamento
...
for i in range(len(Lista_PRECO_fechamento)):
    media_curto_dia = Media_curto[i]  # Valor da média móvel de curto no dia i
    media_longo_dia = Media_longo[i]  # Valor da média móvel de longo no dia i

    if media_curto_dia >= media_longo_dia:  # Compro
        if not Posicionamento.esta_comprado():
            Preco_compra = Lista_PRECO_fechamento[i]
            Lista_preco_compras.append(Preco_compra)
            Posicionamento.comprar()
    else:  # Vendo
        if Posicionamento == 1:
            Preco_venda = Lista_PRECO_fechamento[i]
